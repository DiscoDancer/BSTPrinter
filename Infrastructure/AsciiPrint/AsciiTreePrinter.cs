...
{
    public class AsciiTreePrinter : ITreePrinter
    {
        ...
        {
            Lprofile = GenerateFunctions.GetZeroArray(MaxHeight);
            Rprofile = GenerateFunctions.GetZeroArray(MaxHeight);
        }
        ...
        public static AsciiNode BuildAsciiTree(Node root)
        {
            ...
            static AsciiNode BuildAsciiTreeRecursive(Node root)
            {
                ...
                if (asciiNode.Left != null) asciiNode.Left.ParentDir = -1;

                if (asciiNode.Right != null) asciiNode.Right.ParentDir = 1;
        ...
        private void ComputeLprofile(AsciiNode root, int x, int y)
        {
            var node = root;
        ...
        private void ComputeRprofile(AsciiNode root, int x, int y)
        {
            var node = root;
        ...
        private void ComputeEdgeLengths(AsciiNode root)
        {
            var node = root;
            ...
            else
            {
                ...
                if ((node.Left != null && node.Left.Height == 1 || node.Right != null && node.Right.Height == 1) &&
                    delta > 4) delta -= 1;
            ...
            var h = 1;
            if (node.Left != null) h = Math.Max(node.Left.Height + node.EdgeLength + 1, h);

            if (node.Right != null) h = Math.Max(node.Right.Height + node.EdgeLength + 1, h);

            node.Height = h;
        }

        private void PrintLevel(AsciiNode root, int x, int level)
        {
            var node = root;
